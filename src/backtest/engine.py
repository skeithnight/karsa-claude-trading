...
def backtest_crypto_trend(
    candles: list[dict], ticker: str,
    ema_fast: int = 20, ema_slow: int = 50,
    atr_period: int = 14, atr_sl_mult: float = 2.0, rr_ratio: float = 3.0,
) -> BacktestResult:
    """Crypto Trend + Sentiment Convergence backtest (4h timeframe).

    Entry: Price > EMA_fast > EMA_slow (trend alignment).
    Exit: Close < EMA_fast or 3:1 R/R target hit.
    Stop: entry - ATR * atr_sl_mult.
    """
    result = BacktestResult("Crypto Trend Convergence", ticker, "CRYPTO")
    if len(candles) < max(ema_fast, ema_slow, atr_period) + 5:
        result.failures = ["Insufficient data"]
        return result
    ...
    ema_f = _calc_ema(closes, ema_fast)
    ema_s = _calc_ema(closes, ema_slow)
    ...
    peak = equity = 100.0
    max_dd = 0.0
    start = max(ema_fast, ema_slow, atr_period)

    for i in range(start, len(closes)):
        if not in_position:
            # Long entry: price > fast EMA > slow EMA
            if closes[i] > ema_f[i] > ema_s[i]:
                in_position = True
                entry_price = closes[i]
                atr_val = atr_values[i] if atr_values[i] > 0 else closes[i] * 0.02
                sl = entry_price - atr_val * atr_sl_mult
                tp = entry_price + atr_val * atr_sl_mult * rr_ratio
        else:
            # Exit: close < fast EMA (trend break) or TP/SL hit
            if lows[i] <= sl:
                in_position = False
                pnl_pct = (sl - entry_price) / entry_price * 100
                returns.append(pnl_pct)
                if pnl_pct > 0: wins += 1; gross_profit += pnl_pct
                else: losses += 1; gross_loss += abs(pnl_pct)
                equity *= (1 + pnl_pct / 100)
            elif highs[i] >= tp:
                in_position = False
